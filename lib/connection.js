...
FTP.prototype.put = function(input, path, zcomp, limit, cb) {
  this._store('STOR ' + path, input, zcomp, limit, cb);
};

FTP.prototype.append = function(input, path, zcomp, limit, cb) {
  this._store('APPE ' + path, input, zcomp, limit, cb);
};
...
FTP.prototype.lastMod = function(path, cb) {
  ...
  this._send('MDTM ' + path, function(err, text, code) {
    ...
    // modifiying this to return the data with no timezone offset - DRA 20200415
    ret = new Date(val.year + '-' + val.month + '-' + val.date + 'T' + val.hour
                   + ':' + val.minute + ':' + val.second + '+0000');
    cb(undefined, ret);
  });
};

// custom function to match last modified date with local copy - DRA
FTP.prototype.updateLastMod = function(data, cb) {
  var self = this;
  this._send('MDTM ' + data, function(err, text, code) {
    if (code === 502) {
      return self.list(data, function(err, list) {
        if (err)
          return cb(err);
        if (list.length === 1)
          cb(undefined, list[0].date);
    ...
    if (!val)
      return cb(new Error('Invalid date/time format from server'));
    // updating this to return date with no timezone offset
    ret = new Date(val.year + '-' + val.month + '-' + val.date + 'T' + val.hour
                   + ':' + val.minute + ':' + val.second + '+0000');
    cb(undefined, ret);
  });
...
FTP.prototype._store = function(cmd, input, zcomp, limit, cb) {
